...
const foodObjects = [
  { name: 'omlet', ingredients: ['egg', 'oil'] },
  { name: 'bacon', ingredients: ['egg', 'oil', 'bacon'] }
];
const breakfastSuggestions = {
    'egg+bread+toast': 'Fried Egg with Toast',
    'egg+bacon+bread+toast': 'Egg and Bacon with Toast',
    ...
    'egg+tomato+bread+toast': 'Tomato Omelette with Toast',
    'yogurt+fruit': 'Yogurt and Fruit',
    'yogurt+fruit+cereal': 'Cereal with Yogurt and Fruit',
    'egg+sausage+bread+toast': 'Egg and Sausage with Toast',
...
function getUserIngredients(mealChoice) {
  ...
  const ingredientOptions = {
    breakfast: ['eggs', 'milk', 'bread', 'cereal', 'yogurt', 'fruit', 'bacon', 'sausage', 'butter', 'cheese', 'sugar', 'jam', 'honey', 'corn flakes', 'tomato'],
  ...
  const userInput = rs.question('Enter numbers (e.g., 1, 3, 5):');
    ...
    const matchingObject = findMatchingObject(userInput);
...
function findMatchingObject(ingredients) {
  // Convert the input ingredients to lowercase for case-insensitive matching
  const searchIngredients = ingredients.map(ingredient => ingredient.toLowerCase());

  // Iterate through each object in the array
  for (const foodObject of foodObjects) {
    // Convert the object's ingredients to lowercase for case-insensitive matching
    const objectIngredients = foodObject.ingredients.map(ingredient => ingredient.toLowerCase());

    // Check if all search ingredients are present in the object's ingredients
    if (searchIngredients.every(ingredient => objectIngredients.includes(ingredient))) {
      return foodObject;
    }
  }
